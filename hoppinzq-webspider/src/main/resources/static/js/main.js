...
$(function (){
    ...
    $("#webR_l").buttonLoading().off("click").on("click", function () {
        let $me=$(this);
        let webR=$("#webR").val();
        if(webR==""){
            alert("请输入网址");
        ...
        $me.buttonLoading("start");
        $.get(`${ip}/hoppinzq?method=findResource&params={"url":"${webR}"}`,function (data) {
            $me.buttonLoading("stop");
            ...
            if(m.code==200){
                ...
                let webs=m.data;
                $("#kkt").show();
                $("#wsqzh").off("click").on("click",function () {
                    $(".ordered-list").slideToggle("slow");
                })
                $("#render1").off("click").on("click",function () {
                    $(".ordered-list").fadeIn(500);
                    setTimeout(function () {
                        $(".ordered-list").find("li").each(function (index, element) {
                            var self = $(element);
                            var src=self.text();
                            self.html("")
                            if(self.attr("class")=="png"||self.attr("class")=="jpg"||self.attr("class")=="gif"){
                                self.append("<img class='"+self.attr("class")+"' src='"+src+"'>");
                            }
                            if(self.attr("class")=="mp4"){
                                self.append("<video autoplay muted loop class='mp4'><source src='"+src+"' type='video/mp4'></video>");
                            }
                            if(self.attr("class")=="mp3"){
                                self.append("<audio controls loop class='mp3'>\n" +
                                    "<source src='"+src+"' type='audio/mpeg'>\n" +
                                    "</audio>");
                            }
                            self.append($("<p class='resource_url'></p>").append(src));
                        });
                    },500);
                })
                $.each(webs,function (index,web){
                    var type=web.substring(web.lastIndexOf(".")+1);
                    $(".ordered-list").append(`<li class="${type}">${web}</li>`);
                })

            }else{
                $(".ordered-list").append(`<li><div class="content">
                                                    <h6>出现错误</h6>
                                                    <p>可以向我<a href="contact.html">反馈错误</a></p>
                                                </div></li>`)
            }
    ...
    $("#vitry").click(function () {
        $("#tab-1-55").click()
    })

    $("#webbilibili_l").click(function () {
        $("#webbilibili_l").off("click").on("click",function () {
            alert("只能尝试一次！")
        })
        let bi=$("#webbilibili").val();
        if(bi==""){
            alert("请输入bilibili的网站");
            $("#webbilibili").focus();
            return;
        }
        $.get(`${ip}/hoppinzq?method=checkVideoResourceJson&params={"url":"${bi}","userno":"${userno}"}`);
    })
    $("#mebil").click(function () {
...
function initSocket(){
    ...
    if ('WebSocket' in window) {
        ...
        if (userno == null) {
            userno = uuid(16, 10);//所有页面均使用该参数作为浏览器临时签名,以区别同账户登录但是是不同使用人;
            localStorage.setItem("userno", userno);
        }
        websocket = new WebSocket(wsip+"/websocketProcess/" + userno);
    } else {
        alert('不支持socket')
    }
    ...
    websocket.onmessage = function (event) {
        ...
        if (re.code == 200) {
            if (re.mark == "videoDownLoadMetaMessage") {
                let message = JSON.parse(re.msg);
                console.log(message);
                setMessageInnerHTML(re.msg);
            }else{
                setMessageInnerHTML(re.msg);
            }
        } else {
    ...
    websocket.onclose = function () {
        setMessageInnerHTML("webSocket关闭");
    }
